...
@section('title', 'BudgetNow | Modifica Categoria')
@section('page-title', 'Modifica Categoria')

@section('content')
    <div class="mx-auto max-w-lg">
        <a href="{{ $categoria->entryType ? route('categorie.index', $categoria->entryType) : '#' }}"
                ...
                <div class="mb-5">
                    ...
                    <select id="entry_type_id" name="entry_type_id"
                        ...
                        @foreach($entryTypes as $et)
                            <option value="{{ $et->id }}"
                                    @selected(old('entry_type_id', $categoria->entry_type_id) == $et->id)>
                                {{ $et->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select id="parent_id" name="parent_id"
                        ...
                        @foreach($parents as $parent)
                            <option value="{{ $parent->id }}"
                                    @selected(old('parent_id', $categoria->parent_id) == $parent->id)>
                                {{ $parent->name }}
                            </option>
                        @endforeach
                    </select>
