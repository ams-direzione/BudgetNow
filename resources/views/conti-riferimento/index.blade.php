...
@section('title', 'BudgetNow | Conti di Riferimento')
@section('page-title', 'Conti di Riferimento')
...
$cols = [
    ...
    ['key' => 'account_code', 'label' => 'Codice Conto', 'sortable' => true, 'searchable' => true],
    ['key' => 'bank_name',    'label' => 'Banca',        'sortable' => true, 'searchable' => true],
...
<x-data-table
    ...
    :filters="$filters"
    :per-page="$perPage"
    :create-url="route('conti-riferimento.create')"
    create-label="Aggiungi Conto"
    :colspan-empty="5"
>
    @foreach($accounts as $account)
        ...
        <tbody x-data="editableRow(
                    {{ json_encode($row) }},
                    '{{ route('conti-riferimento.update', $account) }}',
                    '{{ route('conti-riferimento.destroy', $account) }}'
                )"
            ...
            <tr x-show="!editing" class="hover:bg-slate-50 align-middle">
                ...
                <td class="px-4 py-3 font-mono text-slate-600" x-text="orig.account_code"></td>
                <td class="px-4 py-3 text-slate-500" x-text="orig.bank_name || '—'"></td>
                <td class="px-4 py-3 text-slate-500">{{ $account->journalEntries()->count() }}</td>
            ...
            <tr x-show="editing" x-cloak class="bg-amber-50/60 align-top">
                <td class="px-4 py-3">
                    <input type="text" x-model="form.name" placeholder="Nome conto"
                ...
                <td class="px-4 py-3">
                    <input type="text" x-model="form.account_code" placeholder="Codice"
                           class="w-full rounded border font-mono px-2 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                           :class="errors.account_code ? 'border-rose-400 bg-rose-50' : 'border-slate-300'">
                    <p x-show="errors.account_code" x-text="errors.account_code?.[0]" class="text-rose-600 text-xs mt-0.5" x-cloak></p>
                </td>
                <td class="px-4 py-3">
                    <input type="text" x-model="form.bank_name" placeholder="Banca (opzionale)"
